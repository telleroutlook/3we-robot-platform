...
def fig_architecture():
    ...
    ax.text(
        5,
        4.6,
        "Backend Dispatch | Sensor Standardization | Safety Checks",
        ha="center",
        va="center",
        fontsize=6,
        color=COLORS["gray"],
...
def fig_sim2real():
    ...
    colors = [
        COLORS["green"] if r >= t else COLORS["red"] for r, t in zip(ratios, thresholds)
    ]

    bars = ax.bar(
        x, ratios, 0.6, color=colors, alpha=0.85, edgecolor="white", linewidth=0.5
    )
...
def fig_complexity():
    ...
    for i in range(len(tasks)):
        ...
        ax.text(
            mid_x,
            max(ros2_lines[i], threewe_lines[i]) + 8,
            f"{reduction:.0f}×",
            ha="center",
            va="bottom",
            fontsize=7,

...
We present \textbf{3we}, a fully open-source infrastructure that exposes a consistent Python API across a lightweight mock simulator, Gazebo, NVIDIA Isaac Sim, and real hardware (Raspberry Pi~5 + Hailo-8L), so the same user code targets every backend without modification. We make no claim of zero-effort sim-to-real transfer: closing the residual gap (sensor models, dynamics, actuator delay) still requires task-specific tuning, which the platform supports through documented domain-randomization and fault-injection hooks.
...
\begin{itemize}
    ...
    \item \textbf{Consistent API across backends}: a unified Python API where \texttt{Robot(backend="gazebo")} and \texttt{Robot(backend="real")} execute the same user code without modification. Closing the residual sim-to-real gap remains a task-level concern, supported by the platform's noise and fault-injection hooks.
...
3we occupies a unique position: fully open hardware \emph{and} software with a guaranteed-consistent Python API across all backends (an API-level contract; sim-to-real \emph{transfer} still requires task-specific tuning), ROS2-native navigation, and AI-first design. Table~\ref{tab:comparison} summarizes key differences.
...
Cross-backend API & \checkmark & -- & Sim only & -- & Sim only \\
...
These baselines serve as reference points for the public community leaderboard, which is hosted as a versioned JSON file in the repository (\texttt{data/leaderboard.json}) and rendered at \url{https://3we.org/benchmarks}. Submissions are validated against the standardized seed protocol and SDK version. Results are fully reproducible via:
...
\subsection{Domain Randomization and Fault Injection}
...
Beyond sensor-level noise, \texttt{FaultInjector} wraps any backend to simulate communication-layer faults---configurable latency with jitter, stochastic packet loss (returning stale sensor data), and timed disconnections---enabling chaos-engineering tests of policies and ROS2 stacks without modifying user code:
...
Key contributions include: (1)~a Python API that runs unchanged across four backends (mock, Gazebo, Isaac Sim, real hardware), with explicit hooks for closing the residual sim-to-real gap rather than a claim of zero-effort transfer; (2)~reproducible open hardware at approximately \$300 BOM; (3)~AI-first API design reducing navigation code complexity by 12$\times$; (4)~standardized benchmarks with community baselines and a public leaderboard; (5)~Hardware Abstraction Layer supporting third-party robots; (6)~trajectory recording with LeRobot/Open-X format interoperability; and (7)~a domain-randomization module covering both sensor noise and communication-layer fault injection, with Hailo NPU edge VLA inference support.

The project ships 6 Jupyter Notebook tutorials (from Hello World to sim-to-real odometry drift analysis), enabling researchers to immediately verify cross-backend API consistency without hardware.

...
def generate_sim_figure():
    ...
    for i, (wx, wy) in enumerate(waypoints):
        if i == 0:
            ...
            ax.plot(
                wx, wy, "o", color=COLORS["waypoint"], markersize=3, alpha=0.7, zorder=5
            )
    ...
    ax.annotate(
        ...
        xytext=(rx, ry),
        arrowprops=dict(arrowstyle="->", color=COLORS["robot"], lw=1.5),
        zorder=8,
    )
    ...
    legend_elements = [
        plt.Line2D(
            [0], [0], color=COLORS["trajectory"], linewidth=1.2, label="Navigation path"
        ),
        plt.Line2D(
            [0],
            [0],
        ...
        plt.Line2D(
            ...
            marker="o",
            color="none",
            markerfacecolor=COLORS["lidar_hit"],
            markersize=3,
            label="LiDAR hits",
        ),
        mpatches.Patch(

...
\small \url{https://github.com/3we-org/3we-robot-platform}}
...
在物料清单（BOM）成本不超过 300 美元的可复现硬件上，点导航任务的仿真到现实迁移率超过 0.6，社区基线在结构化环境中达到 82\% 的成功率（SPL 0.65）。
...
总体通过标准要求 $\geq$80\% 的测试项目通过。表~\ref{tab:transfer} 中的迁移率数据表明，在仿真中训练的策略在应用记录的噪声模型后，能够在真实硬件上保持有效性。
...
七个标准化场景覆盖多样化环境：\texttt{office\_v2}（20×15m）、\texttt{apartment\_v1}（8×10m）、\texttt{corridor\_v1}（50m 走廊）、\texttt{warehouse\_v1}（40×30m）、\texttt{cluttered\_v1}（12×10m）、\texttt{outdoor\_v1}（50×50m）及 \texttt{dynamic\_v1}（15×15m，含动态障碍物）。
...
本平台已在 \url{https://github.com/3we-org/3we-robot-platform} 公开发布，分别遵循 Apache~2.0（软件）、CERN-OHL-P~v2（硬件）及 CC-BY-SA~4.0（文档）协议。

...
Configuration via ROS2 parameters:
  - broker_url: MQTT broker URL (default: mqtts://localhost:8883, TLS required)
...
class MqttBridgeNode(Node):
    def __init__(self) -> None:
        ...
        self.declare_parameter("broker_url", "mqtts://localhost:8883")
        ...
        if self._broker_url.startswith("mqtt://"):
            self.get_logger().error(
                "Plain mqtt:// is not allowed — control commands must be encrypted. "
                "Use mqtts:// with TLS."
            )
            raise RuntimeError("TLS required: use mqtts:// broker URL")

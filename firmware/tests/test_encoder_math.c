...
void test_encoder_positive_delta_gives_positive_speed(void)
{
    ...
    // Second call with 1440 counts (one full revolution at 50 Hz)
    mock_set_pcnt_count(MOTOR_FL, ENCODER_CPR);
    encoder_update();

    // Expected: (1440 / 1440) * 50 = 50 RPS
    float expected_rps = (float)ENCODER_CPR / (float)ENCODER_CPR * (float)CONTROL_FREQ_HZ;
...
void test_encoder_negative_delta_gives_negative_speed(void)
{
    ...
    // Expected: (-720 / 1440) * 50 = -25 RPS
    float expected_rps = (-720.0f / (float)ENCODER_CPR) * (float)CONTROL_FREQ_HZ;
...
void test_encoder_fractional_revolution(void)
{
    ...
    // 360 counts = quarter revolution
    mock_set_pcnt_count(MOTOR_RL, 360);
    encoder_update();

    float expected_rps = (360.0f / (float)ENCODER_CPR) * (float)CONTROL_FREQ_HZ;

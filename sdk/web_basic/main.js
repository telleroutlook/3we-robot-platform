...
class RobotConnection {
  ...
  connect(url) {
    ...
    this.ws.onopen = () => {
      ...
      this.subscribe("/battery_state", "sensor_msgs/BatteryState", (msg) =>
        this.onBattery(msg),
      );
      this.subscribe("/ultrasonic/front", "sensor_msgs/Range", (msg) =>
        this.onRange("Front", msg),
      );
      this.subscribe("/ultrasonic/back", "sensor_msgs/Range", (msg) =>
        this.onRange("Back", msg),
      );
      this.subscribe("/ultrasonic/left", "sensor_msgs/Range", (msg) =>
        this.onRange("Left", msg),
      );
      this.subscribe("/ultrasonic/right", "sensor_msgs/Range", (msg) =>
        this.onRange("Right", msg),
      );
  ...
  setCmdVel(vx, vy, omega) {
    const scale =
      parseInt(document.getElementById("speedSlider").value) / 100.0;
    const maxV = 0.35;
    const maxW = 2.5;
  ...
  onBattery(msg) {
    ...
    const pct =
      msg.percentage != null ? Math.round(msg.percentage * 100) + "%" : "--%";
...
function toggleEstop() {
  ...
  if (!estopped) {
    ...
    if (
      !confirm(
        "Reset Emergency Stop? Ensure the area is clear before resuming.",
      )
    )
      return;
...
document.addEventListener("keydown", (e) => {
  ...
  switch (e.key.toLowerCase()) {
    case "w":
      robot.setCmdVel(1, 0, 0);
      break;
    case "s":
      robot.setCmdVel(-1, 0, 0);
      break;
    case "a":
      robot.setCmdVel(0, 1, 0);
      break;
    case "d":
      robot.setCmdVel(0, -1, 0);
      break;
    case "q":
      robot.setCmdVel(0, 0, 1);
      break;
    case "e":
      robot.setCmdVel(0, 0, -1);
      break;

...
def main() -> None:
    ...
    sim2real_top = subparsers.add_parser("sim2real", help="Sim2Real validation commands")
    sim2real_sub = sim2real_top.add_subparsers(dest="sim2real_cmd")
    report_parser = sim2real_sub.add_parser("report", help="Generate Sim2Real validation report")
    report_parser.add_argument("--backend", default="gazebo", help="Simulation backend")
    report_parser.add_argument("--scene", default="office_v2", help="Scene to test in")
    report_parser.add_argument("--output", default="reports/", help="Output directory for report")
    report_parser.add_argument("--trials", type=int, default=5, help="Trials per test")
...
def _cmd_sim2real(args: argparse.Namespace) -> None:
    if not hasattr(args, "sim2real_cmd") or args.sim2real_cmd is None:
        print("Usage: threewe sim2real {report}")
        sys.exit(1)

    if args.sim2real_cmd == "report":
        _cmd_sim2real_report(args)


def _cmd_sim2real_report(args: argparse.Namespace) -> None:
    """Generate a Markdown Sim2Real validation report."""
    import time
    from pathlib import Path
    ...
    report_path = output_dir / f"sim2real_{timestamp}.md"

    print(f"Generating Sim2Real report (backend={args.backend}, trials={args.trials})")
    ...
    lines = [
        ...
        f"- **Scene**: {args.scene}",
        f"- **Trials per test**: {args.trials}",
        "",
        "## Results",
        "",
        "| Test | Metric | Sim Value | Pass Condition | Status |",
        "|------|--------|-----------|----------------|--------|",
    ]
    ...
    for test in STANDARD_TRANSFER_TESTS:
        ...
        lines.append(
            f"| {test.name} | {test.metric.value} | {sim_value:.3f} | "
            f"{test.comparison} × {test.pass_multiplier} | {'✅' if result.passed else '❌'} |"
        )
    ...
    lines.extend(
        [
            ...
            "## Summary",
            "",
            f"- **Tests passed**: {passed}/{total} ({pass_rate:.0%})",
    ...
    lines.extend(
        [
            "",
            "---",
            "*Generated by `threewe sim2real report`*",
        ]
    )

    report_path.write_text("\n".join(lines))
    print(f"\n  Report saved: {report_path}")
    print(f"  Result: {passed}/{total} passed ({'PASS' if pass_rate >= 0.8 else 'FAIL'})")
